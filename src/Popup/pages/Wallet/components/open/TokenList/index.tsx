...
export default function TokenList() {
  ...
  const { currentSolanaTokens, removeSolanaToken } = useCurrentSolanaTokens();
  const { data } = useTokensSWR();

  const isExistToken = !!currentSolanaTokens.length;

  const addToken = () => navigate(data.length > 0 ? '/chain/solana/token/add/erc20/search' : '/chain/solana/token/add/erc20');

  function removeEthereumToken(token: any) {
    ...
    <Container>
      <ListTitleContainer>
        <ListTitleLeftContainer>
          ...
          <ListTitleLeftCountContainer>
            <Typography variant="h6">{isExistToken ? `${currentSolanaTokens.length}` : ''}</Typography>
          </ListTitleLeftCountContainer>
        </ListTitleLeftContainer>
        <ListTitleRightContainer>
          {isExistToken && (
      ...
      <ListContainer>
        {isExistToken ? (
          currentSolanaTokens.map((token) => {
            const handleOnClickDelete = async () => {
              await removeSolanaToken(token);
            };
